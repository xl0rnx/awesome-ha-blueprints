...
<Input
  ...
  description='Integrate with Adaptive Lighting custom integration to provide flicker-free powering on experience. Light will be powered with proper parameters, providing instantaneous settings for brightness and color temperature. See Additional Notes for integration link'
  selector='boolean'
/>
<Input
...
If you want to use the [Adaptive Lighting](https://github.com/basnijholt/adaptive-lighting) feature, please select the switch entity created by the Adaptive Lighting integration. 
For example: `switch.adaptive_lighting_bedroom` (If you not sure you can check the entites in settings, the correct on is the one with all attributes).
...
- **2024-02-07**: Add support for the Adaptive Lighting Integration.
