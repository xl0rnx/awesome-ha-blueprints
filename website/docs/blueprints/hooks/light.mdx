...
If you want to use [Adaptive Lighting] (https://github.com/basnijholt/adaptive-lighting) select the switch entity created by the Adaptive Lighting integration. For example: `switch.adaptive_lighting_bedroom` (If you not sure you can check the entites in settings, the correct on is the one with all attributes).
